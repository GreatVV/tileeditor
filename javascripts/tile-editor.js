...
$(document).ready(function() {
	...
	$( "#numberOfColors" ).change(function() {
		_mp.editLevel.changenumberOfColors($( "#numberOfColors" ).val());
	});
	
	$( "#gameMode" ).change(function() {
		_mp.editLevel.changeGameMode();
	});
});
...
_mp.create.navigation = function(){
	...
	//number of colors
	html += '<select id="numberOfColors">';
    html += '<option disabled >Выберите количество цветов</option>';
    html += '<option value="4">4</option>';
	html += '<option value="5">5</option>';
	html += '<option selected="selected" value="6">6</option>';
	html += '<option value="7">7</option>';
	html += '<option value="8">8</option>';
	html += '</select>';
	
	//game mode
	html += '<select id="gameMode">';
    html += '<option selected="selected" disabled >Выберите тип уровня</option>';
    html += '<option value="TargetScore">Набрать очки</option>';
...
_mp.project.addNewLevelButton = function(){
	...
	var tilesWide = 7;
	var tilesHigh = 6 + parseInt(Math.random() * 4);
...
_mp.project.getGameMode = function(mode) {
	var gameMode = {
		Mode : mode, //TargetScore or TargetPlace or TargetChuzzle
		Turns : 30,
		TargetScore : 3000,
		Amount : 20
	}
...
_mp.project.addStage = function(id, minY, maxY, nextStage)
{
	var stage = {
		...
		MinY : minY,
		MaxY : maxY,
		NextStage : nextStage ? nextStage : -1,
		WinOnComplete : false,
		Condition : {
			IsScore : true,
			Target : 200
...
_mp.project.load = function(){
	...
	for(var i = 0; i < _mp.currentProject.levelArray.length; i++)
	{
		...
		if (!level.gameMode)
		{
			level.gameMode = _mp.project.getGameMode("TargetScore");
		}
		if (!level.numberOfColors)
		{
			level.numberOfColors = 6;
		}
		if (!level.stages)
		{
			level.stages = [];
...
_mp.canvas.drawlevel = function(levelIndex){
	...
	if(ElementExists($('.Canvas'))){
		...
		//set gameMode settings for level
		$('#gameMode').val(currentLevelObject.gameMode.Mode);	
		_mp.editLevel.changeGameMode();		
...
_mp.editLevel.changeGameMode = function(numberOfColors) {
	...
	if(!_mp.currentProject.levelArray[_mp.currentLevelIndex]) return;
	var type = $( "#gameMode" ).val();
	
	_mp.currentProject.levelArray[_mp.currentLevelIndex].gameMode.Mode = type;
	if ("#gameModeOption")
	{	
		//unbind events
		$('#turns').unbind( "change" )
		$('#targetScore').unbind( "change" )
		$('#targetAmount').unbind( "change" )
		
		$("#gameModeOption").remove();		
	}
	switch (type) {
		case "TargetScore":
			$('#gameMode').after(function() {
			return '<form id="gameModeOption">Ходы: <input id="turns" type="number" value="30"></input>Целевые очки<input id="targetScore" type="number" value="3000"></input></form>';
			});
			//bind events	
			$('#turns').change(function() {
				_mp.editLevel.turnsChanged($('#turns').val());
			});
			
			$('#targetScore').change(function() {
				_mp.editLevel.targetScoreChanged($('#targetScore').val());
			});
		...
		case "TargetPlace":
			$('#gameMode').after(function() {
				return '<form id="gameModeOption">Ходы: <input id="turns" type="number" value="30"></input>Разместите на поле клетки 2го типа</form>';
			});
			//bind events			
			$('#turns').change(function() {
				_mp.editLevel.turnsChanged($('#turns').val());
		...
		case "TargetChuzzle":
			$('#gameMode').after(function() {
				return '<form id="gameModeOption">Ходы: <input id="turns" type="number" value="30"></input>Целевые количество уничтожений<input id="targetAmount" type="number" value="10"></input></form> Для указания места появления тайла установите тайл 3го типа. Место должно быть только одно';
			});
			//bind events	
			$('#turns').change(function() {
				_mp.editLevel.turnsChanged($('#turns').val());
			...
			$('#targetAmount').change(function() {
				_mp.editLevel.targetAmountChanged($('#targetAmount').val());
			});
		break	  
		
		default:
			alert("Некорректный тип уровня");
		break
	}	
}
_mp.editLevel.turnsChanged = function(turns){
	if(!_mp.currentProject.levelArray[_mp.currentLevelIndex]) return;
